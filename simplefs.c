...
void fcb_block_init(){
  ...
  //  fcb_block_ptr->inodes[32];
     for (int i = 0; i < 4; i++)
    {
        for (int j = 0; j < 32; j++)
        {
            fcb_block_ptr->inodes[j].usedStatus = 0;
            fcb_block_ptr->inodes[j].indexNodeNo;
...
int get_empty_inodeNo_in_fcb(){
    struct superBlock  * fcb_block_ptr;
     fcb_block_ptr = (struct superBlock *)malloc(sizeof(struct superBlock));
     read_block(fcb_block_ptr,0);
      int tmp = fcb_block_ptr->freeinode;
      fcb_block_ptr->freeinode = fcb_block_ptr->freeinode +1;
    write_block(fcb_block_ptr,0);
    return tmp;
}
...
void add_fcb_block(int inodeNo,struct inode * output_ptr){
 struct FCB_block  * fcb_block_ptr;
 fcb_block_ptr =(struct FCB_block *)malloc(sizeof(struct FCB_block));
     int block_location = (inodeNo / 32)+9;
     int offset = inodeNo % 32;
...
int sfs_create(char *filename)
{
    ...
    int directory_enrty_no=targetlocation;
...
void directory_block_init(){
    ...
    current_entry_block->entryList[32];
